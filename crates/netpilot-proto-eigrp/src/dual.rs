...
impl TopologyTable {
    ...
    pub fn get_mut(&mut self, prefix: &str) -> &mut TopologyEntry {
        self.entries.entry(prefix.to_string()).or_insert_with(|| TopologyEntry::new(prefix))
    }
    ...
    pub fn dual(&mut self, prefix: &str, from_neighbor: &str, reported_distance: u32, metric: &EigrpMetric) -> DualResult {
        let entry = self.get_mut(prefix);
        entry.reported_distance.insert(from_neighbor.to_string(), reported_distance);
        ...
        if reported_distance < entry.feasible_distance {
            ...
            DualResult {
                new_successor: None,
                state: DualState::Active { prefix: prefix.to_string() },
            }
        }
    }
...
mod tests {
    ...
    fn dual_accepts_first_route() {
        ...
        let metric = EigrpMetric { bandwidth: 1000, delay: 100, ..Default::default() };
    ...
    fn dual_feasibility_condition_rejects_higher_rd() {
        let mut table = TopologyTable::new();
        let m1 = EigrpMetric { bandwidth: 10, delay: 1, ..Default::default() };
        let m2 = EigrpMetric { bandwidth: 10, delay: 1, ..Default::default() };
    ...
    fn dual_accepts_lower_reported_distance_as_feasible_successor() {
        let mut table = TopologyTable::new();
        let m1 = EigrpMetric { bandwidth: 1000, delay: 100, ..Default::default() };
        let m2 = EigrpMetric { bandwidth: 500, delay: 50, ..Default::default() };
