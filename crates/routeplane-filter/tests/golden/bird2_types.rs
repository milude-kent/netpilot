...
fn golden_bgppath_prepend_adds_to_front() {
    let mut path = AsPath { segments: vec![AsPathSegment::AsSequence(vec![64501])] };
    path.prepend(64500);
    assert_eq!(path.first(), Some(64500));
...
fn golden_bgpmask_asterisk_matches_any_single_asn() {
    let mask = AsPathMask { patterns: vec![AsMaskPattern::Any, AsMaskPattern::Exact(64500)] };
    assert!(mask.matches(&AsPath { segments: vec![AsPathSegment::AsSequence(vec![64999, 64500])] }));
    assert!(!mask.matches(&AsPath { segments: vec![AsPathSegment::AsSequence(vec![64999, 64998])] }));
}

#[test]
fn golden_bgpmask_question_is_optional() {
    let mask = AsPathMask { patterns: vec![AsMaskPattern::Exact(64500), AsMaskPattern::AnyOptional, AsMaskPattern::Exact(64502)] };
    assert!(mask.matches(&AsPath { segments: vec![AsPathSegment::AsSequence(vec![64500, 64502])] }));
    assert!(mask.matches(&AsPath { segments: vec![AsPathSegment::AsSequence(vec![64500, 64999, 64502])] }));
}

#[test]
fn golden_bgpmask_plus_matches_one_or_more() {
    let mask = AsPathMask { patterns: vec![AsMaskPattern::Exact(64500), AsMaskPattern::OneOrMore] };
    assert!(mask.matches(&AsPath { segments: vec![AsPathSegment::AsSequence(vec![64500, 64501])] }));
    assert!(mask.matches(&AsPath { segments: vec![AsPathSegment::AsSequence(vec![64500, 64501, 64502])] }));
    assert!(!mask.matches(&AsPath { segments: vec![AsPathSegment::AsSequence(vec![64500])] }));
}
...
fn golden_rd_type1_display() {
    let rd = RouteDistinguisher::Type1 { ip: Ipv4Addr::new(192, 0, 2, 1), assigned: 100 };
...
fn golden_ec_display() {
    let ec = EcValue { kind: 2, key: 0, value: 64500 };
    assert_eq!(format!("{}", FilterValue::Ec(ec)), "(2,0,64500)");
}

#[test]
fn golden_lc_display() {
    let lc = LcValue { asn: 64500, data1: 1, data2: 100 };
...
fn golden_print_with_newline() {
    assert_eq!(print(&[FilterValue::String("route".into()), FilterValue::Int(42)]), "route 42\n");
}
...
fn golden_evpn_mac_prefix() {
    let prefix = PrefixData {
        nettype: Nettype::EvpnMac, ip: IpAddr::V4(Ipv4Addr::new(10, 0, 0, 1)), length: 32,
        mac: Some([0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff]), evpn_type: Some(2),
        evpn_tag: Some(100), evpn_esi: Some([0x01; 10]),
        router_ip: Some(IpAddr::V4(Ipv4Addr::new(192, 0, 2, 1))),
        rd: None, source_ip: None, source_length: None, maxlen: None, asn: None, vlan_id: None,
    };
